"""
ConvLSTM  — scheduled-sampling (ε @ epoch10 = 0.90)
"""
...
# ε-scheduler (epoch10 → 0.9)
TAU = 120
def get_eps(e):
    if e <= 10:                      # 선형 1.0 → 0.9
        return np.float32(1.0 - 0.01 * e)
    return np.float32(0.9 * np.exp(-(e-10)/TAU))

# Split
files = [f'vorticity{c}_rot{ang}_trimmed.npy'
...
def make_dataset(file_list, shuffle):
...
for epoch in range(1, EPOCHS+1):
    ...
    if val_loss < best_val:
        ...
        model.save('best_sched_ep10_09.keras')
        print("model saved.")
    else:
        wait += 1
